...
$query_cat = $pdo->query("SELECT * FROM os where veterinario = '$_SESSION[cpf_usuario]' and concluido = 'Sim' ");
...
$query_cat = $pdo->query("SELECT * FROM comissoes where data = curDate() and veterinario = '$_SESSION[cpf_usuario]' ");
$res_cat = $query_cat->fetchAll(PDO::FETCH_ASSOC);
...
$totalComissoesMes = 0;
$query_cat = $pdo->query("SELECT * FROM comissoes where data >= '$dataInicioMes' and data <= curDate() and veterinario = '$_SESSION[cpf_usuario]' ");
$res_cat = $query_cat->fetchAll(PDO::FETCH_ASSOC);
for ($i=0; $i < @count($res_cat); $i++) { 
	foreach ($res_cat[$i] as $key => $value) {
	}
...
<div class="row">
	...
	$query_cat = $pdo->query("SELECT * FROM os where concluido != 'Sim' and veterinario = '$_SESSION[cpf_usuario]' order by data_entrega asc, id asc limit 12");
	$res_cat = $query_cat->fetchAll(PDO::FETCH_ASSOC);
	for ($i=0; $i < @count($res_cat); $i++) { 
		foreach ($res_cat[$i] as $key => $value) {
		}
		...
		<div class="col-xl-3 col-md-6 mb-4">
			<div class="card <?php echo $classe2 ?> shadow h-100 py-2">
				<div class="card-body">
					<div class="row no-gutters align-items-center">
						<div class="col mr-2">
							<div class="text-xs font-weight-bold  <?php echo $classe ?> text-uppercase"><?php echo $tipo_animal . ' - ' .$raca ?></div>
